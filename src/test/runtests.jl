...
@testset "Test Set 1: Unit Tests" begin
	...
	@test MyProject.compute_rv_pa(100,100, epoch, 0, 9.905548, 51.54548, 0.15, "optical", moon_r = 0.0)[1] > MyProject.compute_rv_pa(50,50, epoch, 0, 9.905548, 51.54548, 0.15, "optical", moon_r = 0.0)[1]
end
...
@testset "Test Set 2: Integration Tests" begin
	...
	#above test for parallel code
	@test isnan(MyProject.compute_rv_pa(100,100, epoch, 0, 9.905548, 51.54548, 0.15, "optical", moon_r = 696340*5.0)[1])
	...
	#above test for parallel code
	max = MyProject.compute_rv_pa(100,100, epoch, 0, 9.905548, 51.54548, 0.15, "optical", moon_r = 0.0)[1]
	first = MyProject.compute_rv_pa(100,100, 4.8010716718560225e8, 0, 9.905548, 51.54548, 0.15, "optical", moon_r = 0.0)[1]
	@test isapprox(max,first; rtol = 0.1)
	...
	max_parallel = MyProject.compute_rv_pa(100,100, epoch, 0, 9.905548, 51.54548, 0.15, "optical", moon_r = 0.0)[1]
