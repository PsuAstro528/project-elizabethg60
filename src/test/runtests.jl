...
@testset "Test Set 1: Unit Tests" begin
	...
	@test MyProject.compute_rv(100,100, epoch, 0, moon_r = 0.0)[1] > MyProject.compute_rv(50,50, epoch, 0, moon_r = 0.0)[1]
...
@testset "Test Set 2: Integration Tests" begin
    ...
    @test isnan(MyProject.compute_rv(100,100, epoch, 0, moon_r = 696340*5.0)[1])

	#confirm that is no moon present then recovered velocity approx the same at max epoch and first timestamp 
	max = MyProject.compute_rv(100,100, epoch, 0, moon_r = 0.0)[1]
	first = MyProject.compute_rv(100,100, 4.8010716718560225e8, 0, moon_r = 0.0)[1]
